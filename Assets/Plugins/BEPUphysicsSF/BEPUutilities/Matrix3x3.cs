...
namespace BEPUutilities
{
    ...
    public struct Matrix3x3
    {
        ...
        public Matrix3x3(fint m11, fint m12, fint m13, fint m21, fint m22, fint m23, fint m31, fint m32, fint m33)
        {
            M11 = m11;
            M12 = m12;
            M13 = m13;
        ...
        public static Matrix3x3 Identity
        {
            get { return new Matrix3x3(
                1, (fint)0, (fint)0,
                (fint)0, (fint)1, (fint)0,
                (fint)0, (fint)0, (fint)1
                ); }
        }
        ...
        public static void Add(ref Matrix3x3 a, ref Matrix3x3 b, out Matrix3x3 result)
        {
            ...
            fint m31 = a.M31 + b.M31;
            fint m32 = a.M32 + b.M32;
            fint m33 = a.M33 + b.M33;
        ...
        public static void CreateScale(fint scale, out Matrix3x3 matrix)
        {
        ...
        public static Matrix3x3 CreateScale(fint scale)
        {
        ...
        /// <param name="matrix">Scaling matrix.</param>
        public static void CreateScale(fint x, fint y, fint z, out Matrix3x3 matrix)
        {
        ...
        /// <param name="z">Scaling along the z axis.</param>
        /// <returns>Scaling matrix.</returns>
        public static Matrix3x3 CreateScale(fint x, fint y, fint z)
        {
        ...
        {
            fint determinantInverse = (fint)1 / matrix.Determinant();
            fint m11 = (matrix.M22 * matrix.M33 - matrix.M23 * matrix.M32) * determinantInverse;
            fint m12 = (matrix.M13 * matrix.M32 - matrix.M33 * matrix.M12) * determinantInverse;
            fint m13 = (matrix.M12 * matrix.M23 - matrix.M22 * matrix.M13) * determinantInverse;

            fint m21 = (matrix.M23 * matrix.M31 - matrix.M21 * matrix.M33) * determinantInverse;
            fint m22 = (matrix.M11 * matrix.M33 - matrix.M13 * matrix.M31) * determinantInverse;
            fint m23 = (matrix.M13 * matrix.M21 - matrix.M11 * matrix.M23) * determinantInverse;

            fint m31 = (matrix.M21 * matrix.M32 - matrix.M22 * matrix.M31) * determinantInverse;
            fint m32 = (matrix.M12 * matrix.M31 - matrix.M11 * matrix.M32) * determinantInverse;
            fint m33 = (matrix.M11 * matrix.M22 - matrix.M12 * matrix.M21) * determinantInverse;
        ...
        {
            int submatrix;
            fint determinant = matrix.AdaptiveDeterminant(out submatrix);
            fint determinantInverse = (submatrix >= 0) ? ((fint)(1) / determinant) : 0;
            fint m11, m12, m13, m21, m22, m23, m31, m32, m33;
            switch (submatrix)
            {
                case 0: //Full matrix.
        ...
        public static void AdjugateTranspose(ref Matrix3x3 matrix, out Matrix3x3 result)
        {
            ...
            fint m11 = (matrix.M22 * matrix.M33 - matrix.M23 * matrix.M32);
            fint m12 = (matrix.M13 * matrix.M32 - matrix.M33 * matrix.M12);
            fint m13 = (matrix.M12 * matrix.M23 - matrix.M22 * matrix.M13);

            fint m21 = (matrix.M23 * matrix.M31 - matrix.M21 * matrix.M33);
            fint m22 = (matrix.M11 * matrix.M33 - matrix.M13 * matrix.M31);
            fint m23 = (matrix.M13 * matrix.M21 - matrix.M11 * matrix.M23);

            fint m31 = (matrix.M21 * matrix.M32 - matrix.M22 * matrix.M31);
            fint m32 = (matrix.M12 * matrix.M31 - matrix.M11 * matrix.M32);
            fint m33 = (matrix.M11 * matrix.M22 - matrix.M12 * matrix.M21);
        ...
        /// <returns>Product of the multiplication.</returns>
        public static Matrix3x3 operator *(Matrix3x3 m, fint f)
        {
            Matrix3x3 result;
        ...
        /// <param name="f">Scaling value to apply to all components of the matrix.</param>
        /// <returns>Product of the multiplication.</returns>
        public static Matrix3x3 operator *(fint f, Matrix3x3 m)
        {
            Matrix3x3 result;
            Multiply(ref m, f, out result);
            return result;
        ...
        public static void Multiply(ref Matrix3x3 a, ref Matrix3x3 b, out Matrix3x3 result)
        {
            ...
            fint resultM31 = a.M31 * b.M11 + a.M32 * b.M21 + a.M33 * b.M31;
            fint resultM32 = a.M31 * b.M12 + a.M32 * b.M22 + a.M33 * b.M32;
            fint resultM33 = a.M31 * b.M13 + a.M32 * b.M23 + a.M33 * b.M33;
        ...
        {
            fint resultM11 = transpose.M11 * matrix.M11 + transpose.M21 * matrix.M21 + transpose.M31 * matrix.M31;
            fint resultM12 = transpose.M11 * matrix.M12 + transpose.M21 * matrix.M22 + transpose.M31 * matrix.M32;
            fint resultM13 = transpose.M11 * matrix.M13 + transpose.M21 * matrix.M23 + transpose.M31 * matrix.M33;

            fint resultM21 = transpose.M12 * matrix.M11 + transpose.M22 * matrix.M21 + transpose.M32 * matrix.M31;
            fint resultM22 = transpose.M12 * matrix.M12 + transpose.M22 * matrix.M22 + transpose.M32 * matrix.M32;
            fint resultM23 = transpose.M12 * matrix.M13 + transpose.M22 * matrix.M23 + transpose.M32 * matrix.M33;

            fint resultM31 = transpose.M13 * matrix.M11 + transpose.M23 * matrix.M21 + transpose.M33 * matrix.M31;
            fint resultM32 = transpose.M13 * matrix.M12 + transpose.M23 * matrix.M22 + transpose.M33 * matrix.M32;
            fint resultM33 = transpose.M13 * matrix.M13 + transpose.M23 * matrix.M23 + transpose.M33 * matrix.M33;
        ...
        {
            fint resultM11 = matrix.M11 * transpose.M11 + matrix.M12 * transpose.M12 + matrix.M13 * transpose.M13;
            fint resultM12 = matrix.M11 * transpose.M21 + matrix.M12 * transpose.M22 + matrix.M13 * transpose.M23;
            fint resultM13 = matrix.M11 * transpose.M31 + matrix.M12 * transpose.M32 + matrix.M13 * transpose.M33;

            fint resultM21 = matrix.M21 * transpose.M11 + matrix.M22 * transpose.M12 + matrix.M23 * transpose.M13;
            fint resultM22 = matrix.M21 * transpose.M21 + matrix.M22 * transpose.M22 + matrix.M23 * transpose.M23;
            fint resultM23 = matrix.M21 * transpose.M31 + matrix.M22 * transpose.M32 + matrix.M23 * transpose.M33;

            fint resultM31 = matrix.M31 * transpose.M11 + matrix.M32 * transpose.M12 + matrix.M33 * transpose.M13;
            fint resultM32 = matrix.M31 * transpose.M21 + matrix.M32 * transpose.M22 + matrix.M33 * transpose.M23;
            fint resultM33 = matrix.M31 * transpose.M31 + matrix.M32 * transpose.M32 + matrix.M33 * transpose.M33;
        ...
        public static void Multiply(ref Matrix3x3 matrix, fint scale, out Matrix3x3 result)
        {
        ...
        /// <returns>The matrix's determinant.</returns>
        internal fint AdaptiveDeterminant(out int subMatrixCode)
        {
            //Try the full matrix first.
            fint determinant = M11 * M22 * M33 + M12 * M23 * M31 + M13 * M21 * M32 -
                                M31 * M22 * M13 - M32 * M23 * M11 - M33 * M21 * M12;
            if (determinant != (fint)0) //This could be a little numerically flimsy.  Fortunately, the way this method is used, that doesn't matter!
            {
                subMatrixCode = 0;
                return determinant;
            }
        ...
        {
            fint qX2 = quaternion.X + quaternion.X;
            fint qY2 = quaternion.Y + quaternion.Y;
            fint qZ2 = quaternion.Z + quaternion.Z;
            fint XX = qX2 * quaternion.X;
            fint YY = qY2 * quaternion.Y;
            fint ZZ = qZ2 * quaternion.Z;
            fint XY = qX2 * quaternion.Y;
            fint XZ = qX2 * quaternion.Z;
            fint XW = qX2 * quaternion.W;
            fint YZ = qY2 * quaternion.Z;
            fint YW = qY2 * quaternion.W;
            fint ZW = qZ2 * quaternion.W;

            result.M11 = (fint)1 - YY - ZZ;
            result.M21 = XY - ZW;
            result.M31 = XZ + YW;

            result.M12 = XY + ZW;
            result.M22 = (fint)1 - XX - ZZ;
            result.M32 = YZ - XW;

            result.M13 = XZ - YW;
            result.M23 = YZ + XW;
            result.M33 = (fint)1 - XX - YY;
        }
        ...
        public static Matrix3x3 CreateFromAxisAngle(Vector3 axis, fint angle)
        {
            Matrix3x3 toReturn;
        ...
        public static void CreateFromAxisAngle(ref Vector3 axis, fint angle, out Matrix3x3 result)
        {
            fint xx = axis.X * axis.X;
            fint yy = axis.Y * axis.Y;
            fint zz = axis.Z * axis.Z;
            fint xy = axis.X * axis.Y;
            fint xz = axis.X * axis.Z;
            fint yz = axis.Y * axis.Z;

            fint sinAngle = fint.Sin(angle);
            fint oneMinusCosAngle = (fint)1 - fint.Cos(angle);

            result.M11 = (fint)1 + oneMinusCosAngle * (xx - (fint)1);
            ...
            result.M22 = (fint)1 + oneMinusCosAngle * (yy - (fint)1);
            ...
            result.M33 = (fint)1 + oneMinusCosAngle * (zz - (fint)1);
        }
