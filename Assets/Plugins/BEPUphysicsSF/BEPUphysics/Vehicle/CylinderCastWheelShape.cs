...
namespace BEPUphysics.Vehicle
{
    ...
    public class CylinderCastWheelShape : WheelShape
    {
        ...
        public CylinderCastWheelShape(sfloat radius, sfloat width, Quaternion localWheelOrientation, Matrix localGraphicTransform, bool includeSteeringTransformInCast)
        {
        ...
        /// </summary>
        public override sealed sfloat Radius
        {
            get { return shape.Radius; }
            set
            {
                shape.Radius = MathHelper.Max(value, sfloat.Zero);
                Initialize();
            }
        }
        ...
        public sfloat Width
        {
            ...
            set
            {
                shape.Height = MathHelper.Max(value, sfloat.Zero);
                Initialize();
            }
        }
        ...
        public override void UpdateWorldTransform()
        {
            ...
            sfloat length = wheel.suspension.currentLength;
        ...
        protected internal override bool FindSupport(out Vector3 location, out Vector3 normal, out sfloat suspensionLength, out Collidable supportingCollidable, out Entity entity, out Material material)
        {
            suspensionLength = sfloat.MaxValue;
            ...
            if (hit)
            {
                if (suspensionLength > sfloat.Zero)
                {
                    sfloat dot;
        ...
        protected internal override void Initialize()
        {
            ...
            if (expansion.X > sfloat.Zero)
                boundingBox.Max.X += expansion.X;
            else if (expansion.X < sfloat.Zero)
                boundingBox.Min.X += expansion.X;

            if (expansion.Y > sfloat.Zero)
                boundingBox.Max.Y += expansion.Y;
            else if (expansion.Y < sfloat.Zero)
                boundingBox.Min.Y += expansion.Y;

            if (expansion.Z > sfloat.Zero)
                boundingBox.Max.Z += expansion.Z;
            else if (expansion.Z < sfloat.Zero)
        ...
        protected internal override void UpdateDetectorPosition()
        {
            ...
            newPosition.X = wheel.suspension.worldAttachmentPoint.X + wheel.suspension.worldDirection.X * wheel.suspension.restLength * sfloat.Half;
            newPosition.Y = wheel.suspension.worldAttachmentPoint.Y + wheel.suspension.worldDirection.Y * wheel.suspension.restLength * sfloat.Half;
            newPosition.Z = wheel.suspension.worldAttachmentPoint.Z + wheel.suspension.worldDirection.Z * wheel.suspension.restLength * sfloat.Half;
