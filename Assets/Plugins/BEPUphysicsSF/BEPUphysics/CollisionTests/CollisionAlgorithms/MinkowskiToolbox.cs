...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class MinkowskiToolbox
    {
        ...
        public static void ExpandMinkowskiSum(fint marginA, fint marginB, ref Vector3 direction, out Vector3 contribution)
        {
            fint lengthSquared = direction.LengthSquared();
            if (lengthSquared > Toolbox.Epsilon)
            {
                //The contribution to the minkowski sum by the margin is:
                //direction * marginA - (-direction) * marginB.
                Vector3.Multiply(ref direction, (marginA + marginB) / fint.Sqrt(lengthSquared), out contribution);
        ...
        public static void ExpandMinkowskiSum(fint marginA, fint marginB, Vector3 direction, ref Vector3 toExpandA, ref Vector3 toExpandB)
        {
            fint lengthSquared = direction.LengthSquared();
            if (lengthSquared > Toolbox.Epsilon)
            {
                lengthSquared = (fint)1 / fint.Sqrt(lengthSquared);   
                //The contribution to the minkowski sum by the margin is:
                //direction * marginA - (-direction) * marginB. 
