...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class MinkowskiToolbox
    {
        ...
        public static void ExpandMinkowskiSum(sfloat marginA, sfloat marginB, ref Vector3 direction, out Vector3 contribution)
        {
            sfloat lengthSquared = direction.LengthSquared();
            if (lengthSquared > Toolbox.Epsilon)
            {
                //The contribution to the minkowski sum by the margin is:
                //direction * marginA - (-direction) * marginB.
                Vector3.Multiply(ref direction, (marginA + marginB) / libm.sqrtf(lengthSquared), out contribution);
        ...
        public static void ExpandMinkowskiSum(sfloat marginA, sfloat marginB, Vector3 direction, ref Vector3 toExpandA, ref Vector3 toExpandB)
        {
            sfloat lengthSquared = direction.LengthSquared();
            if (lengthSquared > Toolbox.Epsilon)
            {
                lengthSquared = sfloat.One / libm.sqrtf(lengthSquared);   
                //The contribution to the minkowski sum by the margin is:
                //direction * marginA - (-direction) * marginB. 
