...
namespace BEPUphysics.Character
{
    ...
    public class HorizontalMotionConstraint : SolverUpdateable
    {
        ...
        public Vector2 MovementDirection
        {
            ...
            set
            {
                if (movementDirection.X != value.X || movementDirection.Y != value.Y) //Floating point comparison is perfectly fine here. Any bitwise variation should go through.
                {
                    ...
                    {
                        Vector2.Divide(ref value, libm.sqrtf(lengthSquared), out movementDirection);
                    }
                    else
                    {
        ...
        /// </summary>
        public sfloat MaximumAccelerationForce { get; set; }
        sfloat maxForceDt;
        sfloat maxAccelerationForceDt;

        private sfloat timeUntilPositionAnchor = (sfloat).2f;
        ...
        /// </summary>
        public sfloat TimeUntilPositionAnchor
        {
        ...
        /// </summary>
        public sfloat PositionAnchorDistanceThreshold { get; set; }
        ...
        {
            get { return timeSinceTransition < sfloat.Zero; }
        }
        ...
        public void UpdateMovementBasis(ref Vector3 forward)
        {
            ...
            sfloat forwardLengthSquared = horizontalForwardDirection.LengthSquared();
            ...
            {
                Vector3.Divide(ref horizontalForwardDirection, libm.sqrtf(forwardLengthSquared), out horizontalForwardDirection);
        ...
        sfloat supportForceFactor = sfloat.One;
        /// <summary>
        ...
        /// </summary>
        public sfloat SupportForceFactor
        {
            get
            {
        ...
        public HorizontalMotionConstraint(Entity characterBody, SupportFinder supportFinder)
        {
            ...
            MaximumAccelerationForce = sfloat.MaxValue;
        }
        ...
        public override void Update(sfloat dt)
        {
            ...
            isTryingToMove = movementDirection3d.LengthSquared() > sfloat.Zero;
            ...
            if (MovementMode != MovementMode.Floating)
            {
                ...
                if (isTryingToMove)
                {
                    ...
                    Plane plane = new Plane(supportData.Normal, sfloat.Zero);
                    sfloat t;
                ...
                else
                {
                    ...
                    sfloat length = linearJacobianA1.LengthSquared();
                    ...
                    Vector3.Divide(ref linearJacobianA1, libm.sqrtf(length), out linearJacobianA1);
            ...
            {
                //If the character is sfloating, then the jacobians are simply the 3d movement direction and the perpendicular direction on the character's horizontal plane.
            ...
            targetVelocity.X = isTryingToMove ? TargetSpeed : sfloat.Zero;
            targetVelocity.Y = sfloat.Zero;

            //Compute the effective mass matrix.
            if (supportEntity != null && supportEntity.IsDynamic)
            {
                sfloat m11, m22, m1221 = sfloat.Zero;
                sfloat inverseMass;
            ...
            if (supportEntity != null && ((wasTryingToMove && !isTryingToMove) || (!hadTraction && supportFinder.HasTraction) || supportEntity != previousSupportEntity))
            {
                ...
                timeSinceTransition = sfloat.Zero;
            }
            ...
            if (!isTryingToMove && MovementMode == MovementMode.Traction && supportEntity != null)
            {
                ...
                if (timeSinceTransition >= sfloat.Zero && timeSinceTransition < timeUntilPositionAnchor)
                ...
                if (timeSinceTransition >= timeUntilPositionAnchor)
                {
                    ...
                    timeSinceTransition = sfloat.MinusOne; //Negative 1 means that the offset has been computed.
                }
                if (timeSinceTransition < sfloat.Zero)
                {
                    ...
                    else
                    {
                        ...
                        Vector2.Multiply(ref positionCorrectionBias, (sfloat).2f / dt, out positionCorrectionBias);
                    }
                }
            }
            else
        ...
        public override sfloat SolveIteration()
        {
            ...
            if (MovementMode == MovementMode.Floating)
            {
                //If it's sfloating, clamping rules are different.
                ...
                accumulatedImpulse.X = MathHelper.Clamp(accumulatedImpulse.X + lambda.X, sfloat.Zero, maxForceDt);
                accumulatedImpulse.Y = sfloat.Zero;
            }
            else
            {
                ...
                {
                    Vector2.Multiply(ref accumulatedImpulse, maxForceDt / libm.sqrtf(length), out accumulatedImpulse);
                }
