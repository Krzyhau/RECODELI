...
namespace BEPUphysics.Character
{
    ...
    public class StepManager
    {
        ...
        /// </summary>
        public sfloat MinimumDownStepHeight
        {
            get
            {
            ...
            set
            {
                if (minimumDownStepHeight < sfloat.Zero)
                    throw new ArgumentException("Value must be nonnegative.");
                minimumDownStepHeight = value;
            }
        }
        sfloat minimumUpStepHeight;
        ...
        public StepManager(Cylinder characterBody, CharacterContactCategorizer contactCategorizer, SupportFinder supportFinder, QueryManager queryManager, HorizontalMotionConstraint horizontalMotionConstraint)
        {
            ...
            minimumUpStepHeight = CollisionDetectionSettings.AllowedPenetration * (sfloat)1.1f;// Math.Max(0, -.01f + character.Body.CollisionInformation.Shape.CollisionMargin * (1 - character.SupportFinder.sinMaximumSlope));
        ...
        public bool TryToStepDown(out Vector3 newPosition)
        {
            ...
            sfloat highestBound = sfloat.Zero;
            ...
            sfloat lowestBound = characterBody.CollisionInformation.Shape.CollisionMargin + SupportFinder.SupportRayData.Value.HitData.T - SupportFinder.BottomDistance;
            sfloat currentOffset = lowestBound;
            sfloat hintOffset;
            ...
            try
            {
                ...
                {
                    currentOffset = hitT + CollisionDetectionSettings.AllowedPenetration * sfloat.Half;
                                                ...
                                                out hintOffset))
                    {
                        ...
                        case CharacterContactPositionState.NoHit:
                            ...
                            currentOffset = (lowestBound + currentOffset) * sfloat.Half;
                            break;
                        case CharacterContactPositionState.Obstructed:
                            lowestBound = currentOffset;
                            currentOffset = (highestBound + currentOffset) * sfloat.Half;
                            break;
                ...
                while (attempts++ < 5 && lowestBound - highestBound > Toolbox.BigEpsilon)
                {
                    ...
                    switch (TryDownStepPosition(ref candidatePosition, ref down, ref tractionContacts, ref supportContacts, ref sideContacts, ref headContacts, out hintOffset))
                    {
                        ...
                        case CharacterContactPositionState.NoHit:
                            highestBound = currentOffset + hintOffset;
                            currentOffset = (lowestBound + highestBound) * sfloat.Half;
                            break;
                        case CharacterContactPositionState.Obstructed:
                            lowestBound = currentOffset;
                            currentOffset = (highestBound + lowestBound) * sfloat.Half;
                            break;
                        case CharacterContactPositionState.TooDeep:
                            currentOffset += hintOffset;
                            lowestBound = currentOffset;
        ...
        CharacterContactPositionState TryDownStepPosition(ref Vector3 position, ref Vector3 down,
            ...
            out sfloat hintOffset)
        {
            hintOffset = sfloat.Zero;
            ...
            if (tractionContacts.Count > 0)
            {
                ...
                else if (supportState == CharacterContactPositionState.TooDeep)
                {
                    ...
                    hintOffset = sfloat.Min(sfloat.Zero, (sfloat).001f - Vector3.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth);
                ...
                else //if (supportState == SupportState.Separated)
                {
                    ...
                    hintOffset = (sfloat)(-.001f) - Vector3.Dot(supportContact.Contact.Normal, down) * supportContact.Contact.PenetrationDepth;
        ...
        sfloat upStepMargin = (sfloat).1f;  //There's a little extra space above the maximum step height to start the obstruction and downcast test rays.  Helps when a step is very close to the max step height.
        void FindUpStepCandidates(RawList<ContactData> outputStepCandidates, ref Vector3 down)
        {
            ...
            foreach (var c in SupportFinder.SideContacts)
            {
                ...
                if (dot > sfloat.Zero)
                {
                    ...
                    if (dot < characterBody.Height * sfloat.Half && dot > characterBody.Height * sfloat.Half - maximumStepHeight - upStepMargin)
                    {
                        //It's a candidate!
        ...
        bool TryToStepUsingContact(ref ContactData contact, ref Vector3 down, out Vector3 newPosition)
        {
            ...
            Ray ray;
            sfloat downRayLength = characterBody.Height;// MaximumStepHeight + upStepMargin;
            Vector3.Multiply(ref down, characterBody.Height * sfloat.Half - downRayLength, out ray.Position);
            ...
            sfloat horizontalOffsetAmount = characterBody.CollisionInformation.Shape.CollisionMargin;// (sfloat)((1 - character.SupportFinder.sinMaximumSlope) * character.Body.CollisionInformation.Shape.CollisionMargin + 0);
            sfloat length = characterBody.Radius + horizontalOffsetAmount;// -contact.PenetrationDepth;
            ...
            if (!QueryManager.RayCast(ray, downRayLength, out earliestHit) || //Can't do anything if it didn't hit.
                earliestHit.T <= sfloat.Zero || //Can't do anything if the hit was invalid.
            ...
            //earliestHitUp.T = sfloat.MaxValue;
            sfloat upLength = characterBody.Height - earliestHit.T;
            ...
            //The words 'highest' and 'lowest' here refer to the position relative to the character's body.
            //The ray cast points downward relative to the character's body.
            sfloat highestBound = -maximumStepHeight;
            sfloat lowestBound = characterBody.CollisionInformation.Shape.CollisionMargin - downRayLength + earliestHit.T;
            sfloat currentOffset = lowestBound;
            sfloat hintOffset;
                ...
                if (Toolbox.GetRayPlaneIntersection(ref downRay, ref plane, out hitT, out intersection))
                {
                                              ...
                                              out hintOffset))
                    {
                        case CharacterContactPositionState.Accepted:
                            currentOffset += hintOffset;
                            //Only use the new position location if the movement distance was the right size.
                            if (currentOffset < sfloat.Zero && currentOffset > -maximumStepHeight - CollisionDetectionSettings.AllowedPenetration)
                            {
                                ...
                                newPosition = characterBody.Position + sfloat.Max(-maximumStepHeight, currentOffset) * down + horizontalOffset;
                                return true;
                            }
                            else
                            {
        ...
        private void PrepareQueryObject(ref Vector3 position)
        {
            ...
            currentQueryObject.UpdateBoundingBoxForTransform(ref transform, sfloat.Zero);
        }
            ...
            if (!obstructed && supportContacts.Count > 0)
            {
                ...
                if (supportState == CharacterContactPositionState.Accepted)
                {
                    if (tractionContacts.Count > 0)
                    {
                        ...
                        hintOffset = sfloat.Min(sfloat.Zero, Vector3.Dot(supportContact.Contact.Normal, down) * (CollisionDetectionSettings.AllowedPenetration * sfloat.Half - supportContact.Contact.PenetrationDepth));
                        return CharacterContactPositionState.Accepted;
                    }
                    else
                    {
                        ...
                        Ray downRay;
                        downRay.Position = supportContact.Contact.Position + sideNormal * (sfloat).001f;
                        sfloat verticalOffset = Vector3.Dot(downRay.Position - position, down);
                        verticalOffset = characterBody.Height * sfloat.Half + verticalOffset;
                        ...
                        obstructionTestRay.Position = position - down * (characterBody.Height * sfloat.Half);
                        obstructionTestRay.Direction = downRay.Position - obstructionTestRay.Position;

                        if (!QueryManager.RayCastHitAnything(obstructionTestRay, sfloat.One))
                        {
                            ...
                            if (QueryManager.RayCast(downRay, characterBody.Height, out hit))
                            {
                                ...
                                if (characterBody.Height - maximumStepHeight < hit.T)
                                {
                                    ...
                                    if (sfloat.Abs(dot) > ContactCategorizer.TractionThreshold)
                                    {
                                        ...
                                        if (QueryManager.RayCast(downRay, characterBody.Height * sfloat.Half + maximumStepHeight, out hit))
                                        {
                                            //It hit.. almost there!
                                            hit.Normal.Normalize();
                                            Vector3.Dot(ref hit.Normal, ref down, out dot);
        ...
        bool IsObstructiveToUpStepping(ref Vector3 sideNormal, ref ContactData contact)
        {
            ...
            foreach (var c in SupportFinder.SideContacts)
            {
                ...
                sfloat depth = dot * c.Contact.PenetrationDepth;
                if (depth > sfloat.Max(c.Contact.PenetrationDepth, CollisionDetectionSettings.AllowedPenetration))
                    return true;
