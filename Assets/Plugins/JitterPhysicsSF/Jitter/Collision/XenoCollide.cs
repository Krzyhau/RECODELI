...
namespace Jitter.Collision
{
    ...
    public sealed class XenoCollide
    {
        ...
        private static sfloat CollideEpsilon = (sfloat)1e-4f;
        ...
        public static bool Detect(ISupportMappable support1, ISupportMappable support2, ref JMatrix orientation1,
             ...
             out JVector point, out JVector normal, out sfloat penetration)
        {
            // Used variables
            JVector temp1, temp2;
            JVector v01, v02, v0;
            ...
            if (v0.IsNearlyZero()) v0 = new JVector((sfloat)0.00001f, sfloat.Zero, sfloat.Zero);
            ...
            if (JVector.Dot(ref v1, ref normal) <= sfloat.Zero) return false;
            ...
            if (normal.IsNearlyZero())
            {
                ...
                JVector.Multiply(ref point, sfloat.Half, out point);
            ...
            if (JVector.Dot(ref v2, ref normal) <= sfloat.Zero) return false;
            ...
            sfloat dist = JVector.Dot(ref normal, ref v0);
            ...
            while (true)
            {
                ...
                if (JVector.Dot(ref v3, ref normal) <= sfloat.Zero)
                {
                    return false;
                }
                ...
                if (JVector.Dot(ref temp1, ref v0) < sfloat.Zero)
                {
                    v2 = v3;
                    v21 = v31;
                    v22 = v32;
                ...
                while (true)
                {
                    ...
                    sfloat d = JVector.Dot(ref normal, ref v1);


                    // If the origin is inside the wedge, we have a hit
                    if (d >= sfloat.Zero && !hit)
                    {
                        // HIT!!!
                        hit = true;
                    }
                    ...
                    sfloat delta = JVector.Dot(ref temp1, ref normal);
                    penetration = JVector.Dot(ref v4, ref normal);

                    // If the boundary is thin enough or the origin is outside the support plane for the newly discovered vertex, then we can terminate
                    if (delta <= CollideEpsilon || penetration <= sfloat.Zero || phase2 > MaximumIterations)
                    {

                        if (hit)
                        {
                            JVector.Cross(ref v1, ref v2, out temp1);
                            sfloat b0 = JVector.Dot(ref temp1, ref v3);
                            JVector.Cross(ref v3, ref v2, out temp1);
                            sfloat b1 = JVector.Dot(ref temp1, ref v0);
                            JVector.Cross(ref v0, ref v1, out temp1);
                            sfloat b2 = JVector.Dot(ref temp1, ref v3);
                            JVector.Cross(ref v2, ref v1, out temp1);
                            sfloat b3 = JVector.Dot(ref temp1, ref v0);

                            sfloat sum = b0 + b1 + b2 + b3;
                            ...
                            JVector.Multiply(ref point, inv * sfloat.Half, out point);
                    ...
                    //sfloat d1 = JVector.Dot(ref temp1, ref v0);


                    //// Compute the tetrahedron dividing face (v4,v0,v2)
                    //JVector.Cross(ref v4, ref v2, out temp1);
                    //sfloat d2 = JVector.Dot(ref temp1, ref v0);


                    // Compute the tetrahedron dividing face (v4,v0,v3)
                    JVector.Cross(ref v4, ref v0, out temp1);
                    sfloat dot = JVector.Dot(ref temp1, ref v1);
