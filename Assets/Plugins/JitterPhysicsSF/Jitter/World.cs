...
namespace Jitter
{
    ...
    public class World
    {
        public delegate void WorldStep(sfloat timestep);
        ...
        public class WorldEvents
        {
            ...
            internal void RaiseWorldPreStep(sfloat timestep)
            {
                if (PreStep != null) PreStep(timestep);
            }

            internal void RaiseWorldPostStep(sfloat timestep)
            {
        ...
        private sfloat inactiveAngularThresholdSq = (sfloat)0.1f;
        private sfloat inactiveLinearThresholdSq = (sfloat)0.1f;
        private sfloat deactivationTime = sfloat.Two;

        private sfloat angularDamping = (sfloat)0.85f;
        private sfloat linearDamping = (sfloat)0.85f;

        private int contactIterations = 10;
        private int smallIterations = 4;
        private sfloat timestep = sfloat.Zero;
        ...
        private JVector gravity = new JVector(sfloat.Zero, (sfloat)(-9.81f), sfloat.Zero);
        ...
        /// The default value is 0.85</param>
        public void SetDampingFactors(sfloat angularDamping, sfloat linearDamping)
        {
            if (angularDamping < sfloat.Zero || angularDamping > sfloat.One)
                throw new ArgumentException("Angular damping factor has to be between 0.0 and 1.0", "angularDamping");

            if (linearDamping < sfloat.Zero || linearDamping > sfloat.One)
        ...
        public void SetInactivityThreshold(sfloat angularVelocity, sfloat linearVelocity, sfloat time)
        {
            if (angularVelocity < sfloat.Zero) throw new ArgumentException("Angular velocity threshold has to " +
                 "be larger than zero", "angularVelocity");

            if (linearVelocity < sfloat.Zero) throw new ArgumentException("Linear velocity threshold has to " +
                "be larger than zero", "linearVelocity");

            if (time < sfloat.Zero) throw new ArgumentException("Deactivation time threshold has to " +
        ...
        private sfloat currentLinearDampFactor = sfloat.One;
        private sfloat currentAngularDampFactor = sfloat.One;
        ...
        public void Step(sfloat timestep, bool multithread)
        {
            this.timestep = timestep;

            // yeah! nothing to do!
            if (timestep == sfloat.Zero) return;

            // throw exception if the timestep is smaller zero.
            if (timestep < sfloat.Zero) throw new ArgumentException("The timestep can't be negative.", "timestep");

            // Calculate this
            currentAngularDampFactor = JMath.Pow(angularDamping, timestep);
            currentLinearDampFactor = JMath.Pow(linearDamping, timestep);
        ...
        private sfloat accumulatedTime = sfloat.Zero;
        ...
        public void Step(sfloat totalTime, bool multithread, sfloat timestep, int maxSteps)
        {
            int counter = 0;
        ...
        private void UpdateArbiterContacts(Arbiter arbiter)
        {
            ...
            for (int i = arbiter.contactList.Count - 1; i >= 0; i--)
            {
                ...
                else
                {
                    ...
                    sfloat distance = JVector.Dot(ref diff, ref c.normal);
                    ...
                    // following line.
                    if (distance > contactSettings.breakThreshold * contactSettings.breakThreshold * (sfloat)100)
                    {
                        Contact.Pool.GiveBack(c);
                        arbiter.contactList.RemoveAt(i);
                        continue;
        ...
        private void IntegrateCallback(object obj)
        {
            ...
            if (!(body.isParticle))
            {
                ...
                //exponential map
                JVector axis;
                sfloat angle = body.angularVelocity.Length();

                if (angle < (sfloat)0.001f)
                {
                    // use Taylor's expansions of sync function
                    // axis = body.angularVelocity * (0.5f * timestep - (timestep * timestep * timestep) * (0.020833333333f) * angle * angle);
                    JVector.Multiply(ref body.angularVelocity, (sfloat.Half * timestep - (timestep * timestep * timestep) * (sfloat)(0.020833333333f) * angle * angle), out axis);
                }
                else
                {
                    // sync(fAngle) = sin(c*fAngle)/t
                    JVector.Multiply(ref body.angularVelocity, (JMath.Sin(sfloat.Half * angle * timestep) / angle), out axis);
                }

                JQuaternion dorn = new JQuaternion(axis.X, axis.Y, axis.Z, JMath.Cos(angle * timestep * sfloat.Half));
